...
chart.data = [
<?php 
$consultag = "select YEAR(fecha_escala), count(*) as cuenta, riesgo_niv from tbl_riesgos group by YEAR(fecha_escala), riesgo_niv";
...
while ($columnag = mysqli_fetch_array( $resultadog ))
	{	?>
{
	"Riesgo": "<?php echo $columnag['riesgo_niv'] ?>",
	"Cantidad": <?php echo $columnag['cuenta'] ?>
		}, 
	<?php	} ?>
];
...
categoryAxis.dataFields.category = "Riesgo";
...
series.dataFields.categoryX = "Riesgo";
series.dataFields.categoryX = "Riesgo";
...
TABLA DE DATOS (Temporal)
	...
	$consulta = "select YEAR(fecha_escala), count(*) as cuenta, riesgo_niv from tbl_riesgos group by YEAR(fecha_escala), riesgo_niv";
	$resultado = mysqli_query( $conexion, $consulta ) or die ( "Error en la Consulta a la BD o Tabla");
	
   // Construccion de tabla
	echo "<table borde='2'>";
	echo "<tr>";
	echo "<th>Nivel de Riesgo</th><th>Cantidad</th><th>A&ntilde;o</th>";
	echo "</tr>";
	// Bucle while que recorre cada registro y muestra cada campo en la tabla.
	while ($columna = mysqli_fetch_array( $resultado ))
	{
		echo "<tr>";
		echo "<td>" . $columna['riesgo_niv'] . "</td><td>" . $columna['cuenta'] . "</td><td>" . $columna['YEAR(fecha_escala)'] . "</td>";
		echo "</tr>";
	}
